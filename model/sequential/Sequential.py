...
# Loading the data using utilities module
dataset = u.load_mnist_dataset

# Preprocessing steps to add single color channel as data is in greyscale for Keras
dataset.x_train.reshape(x_train.shape[0], 28, 28, 1)
dataset.x_test.reshape(x_test.shape[0], 28, 28, 1)
input_shape = (28, 28, 1)

def train_data(dataset:classes.dataset.Dataset):
 
    # convert class vectors to binary class matrices
    num_classes = 10
    y_train = tensorflow.keras.utils.to_categorical(
        y_train, num_classes=num_classes)
    y_test = tensorflow.keras.utils.to_categorical(
